...
const nextAction = (action, data) => ({ ...action, ...data, [BACKEND_CALL]: undefined })
...
function APICall({ endpoint, method, query, payload, headers, type, text }) {
  return new Promise((resolve, reject) => {
    ...
    if (type) r = r.type(type)

    r.then(data => resolve(text ? { text: data.text } : data.body), error => reject(error))
  })
}
